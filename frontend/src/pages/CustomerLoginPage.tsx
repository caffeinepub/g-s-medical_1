...
import { useCustomerLogin } from '../hooks/useQueries';
import { useCustomerAuth } from '../hooks/useCustomerAuth';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { Eye, EyeOff, Lock, Mail, HeartPulse, User, Loader2, CheckCircle } from 'lucide-react';
...
export default function CustomerLoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const token = await customerLogin.mutateAsync({ email, password });
      if (token) {
        login(token);
        navigate({ to: '/' });
      } else {
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-900 via-emerald-800 to-emerald-900 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8 animate-fade-up">
          ...
          <p className="text-emerald-300 text-sm">Customer Portal</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden animate-fade-up" style={{ animationDelay: '0.1s' }}>
          ...
          <div className="bg-gradient-to-r from-emerald-700 to-emerald-800 px-8 py-5 flex items-center gap-3">
            ...
            <div>
              <h2 className="text-white font-semibold text-lg">Customer Login</h2>
              <p className="text-emerald-200 text-xs">Welcome back to G&S MEDICAL</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="px-8 py-7 space-y-5">
            ...
            <div className="text-center pt-1 space-y-2">
              <p className="text-sm text-muted-foreground">
                ...
                <button type="button" onClick={() => navigate({ to: '/customer/signup' })}
